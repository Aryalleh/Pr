...
const Router = {
	...
	async handleApi(request, url, env, ctx) {
		...
		if (portalMatch) {
			...
			if (sub === "/payment" && request.method === "POST") {
				...
				if (settings.payment_card_enabled === "1" && settings.card_number) {
					...
					await env.DB.prepare("UPDATE payments SET card_number=?, card_owner=?, amount_irr=? WHERE id=?").bind(settings.card_number, settings.card_owner || "", smsAmount, paymentId).run();
				...
				if (settings.payment_tetra_enabled === "1" && settings.tetra_api_key) {
					const orderIdStr = "ZEUS-" + paymentId;
		...
		if (url.pathname === "/api/sms" && request.method === "GET") {
			const msg = url.searchParams.get("msg") || "";
			const amount = parseBankSmsAmountRials(msg);
			if (!amount) return new Response(JSON.stringify({ ok: true, parsed: false }), { headers: { "Content-Type": "application/json" } });
			const payment = await env.DB.prepare("SELECT * FROM payments WHERE card_number IS NOT NULL AND status='pending' AND amount_irr = ?").bind(amount).first();
			if (!payment) return new Response(JSON.stringify({ ok: true, parsed: true, matched: false, amount }), { headers: { "Content-Type": "application/json" } });
			await env.DB.prepare("UPDATE payments SET method = 'card' WHERE id = ?").bind(payment.id).run();
			await activateUserPayment(env, payment);
			return new Response(JSON.stringify({ ok: true, parsed: true, matched: true, amount, payment_id: payment.id }), { headers: { "Content-Type": "application/json" } });
		}
		if (url.pathname === "/api/payment/callback" && request.method === "POST") {
			...
			const m = String(b.order_id).match(/^ZEUS-(\d+)$/);
		...
		if (url.pathname === "/api/payments/approve" && request.method === "POST") {
			...
			if (payment.tetra_pay_token && !payment.tetra_tx_id) {
				const apiKeyRow = await env.DB.prepare("SELECT value FROM settings WHERE key = 'tetra_api_key'").first();
				if (apiKeyRow && apiKeyRow.value) await AryallehPay.manualConfirm(apiKeyRow.value, payment.id, "manual admin approve");
			}
			await activateUserPayment(env, payment);
			return new Response(JSON.stringify({ ok: true }), { headers: { "Content-Type": "application/json" } });
		}
