	...
	async handleApi(request, url, env, ctx) {
		...
		if (portalMatch) {
			...
			if (sub === "" && request.method === "GET") {
				...
				return new Response(
					JSON.stringify({
						...
						block_ads: Boolean(user.block_ads),
						block_porn: Boolean(user.block_porn),
						subscription: {
							total_mb: totalMb,
			...
			if (sub === "/settings" && request.method === "POST") {
				const { block_ads, block_porn } = await request.json().catch(() => ({}));
				const updates = [];
				const binds = [];
				if (block_ads !== undefined) {
					updates.push("block_ads = ?");
					binds.push(block_ads ? 1 : 0);
				}
				if (block_porn !== undefined) {
					updates.push("block_porn = ?");
					binds.push(block_porn ? 1 : 0);
				}
				if (!updates.length) return new Response(JSON.stringify({ ok: false, error: "فیلدی برای تغییر ارسال نشده" }), { status: 400, headers: { "Content-Type": "application/json" } });
				binds.push(user.username);
				await env.DB.prepare(`UPDATE users SET ${updates.join(", ")} WHERE username = ?`).bind(...binds).run();
			...
			if (sub === "/payment" && request.method === "POST") {
				...
				if (settings.telegram_bot_token && settings.telegram_channel_id) {
					const notifyTask = async () => {
						try {
							await Telegram.call(settings.telegram_bot_token, "sendMessage", {
								chat_id: settings.telegram_channel_id,
								text:
									"🧾 پرداخت جدید\n" +
									"کاربر: " + (user.name || user.username) + " (" + user.username + ")\n" +
									"بسته: " + pkg.name + "\n" +
									"مبلغ: " + priceToman.toLocaleString("fa-IR") + " تومان\n" +
									"شماره سفارش: " + orderIdStr,
								reply_markup: { inline_keyboard: [[{ text: "✅ تایید پرداخت", callback_data: "appr:" + paymentId }]] },
							});
						} catch (e) {}
					};
					if (ctx) ctx.waitUntil(notifyTask());
					else await notifyTask();
				}
				return new Response(JSON.stringify({ ok: true, payment_id: paymentId, methods, expires_at: expiresAt, price_irr: priceToman }), { headers: { "Content-Type": "application/json" } });
			}
		...
		if (url.pathname === "/api/payment/callback" && request.method === "POST") {
			...
			let payment = await env.DB.prepare("SELECT * FROM payments WHERE order_id = ?").bind(String(b.order_id)).first();
			if (!payment) {
				const m = String(b.order_id).match(/^CRAB-(\d+)$/);
				if (m) payment = await env.DB.prepare("SELECT * FROM payments WHERE id = ?").bind(Number(m[1])).first();
			}
			...
			await env.DB.prepare("UPDATE payments SET tetra_tx_id = ?, method = 'tetra' WHERE id = ?").bind(String(b.tx_id || ""), payment.id).run();
		...
		if (url.pathname === "/api/telegram/webhook" && request.method === "POST") {
			...
			if (cb && cb.data && cb.data.startsWith("appr:")) {
				const paymentId = Number(cb.data.slice(5));
				const payment = await env.DB.prepare("SELECT * FROM payments WHERE id = ?").bind(paymentId).first();
				if (!payment) {
					await Telegram.call(tgSettings.telegram_bot_token, "answerCallbackQuery", { callback_query_id: cb.id, text: "پرداخت یافت نشد", show_alert: true });
					return new Response("ok");
				}
				...
				if (cb.message) {
					...
					await Telegram.call(tgSettings.telegram_bot_token, "editMessageText", {
						chat_id: cb.message.chat.id,
						message_id: cb.message.message_id,
						text: (cb.message.text || "") + "\n\n✅ تایید شد توسط " + approverName,
					});
				}
			}
			return new Response("ok");
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				...
				const githubRes = await fetch("https://raw.githubusercontent.com/Aryalleh/Pr/refs/heads/main/crabvpn.js?t=" + Date.now() + Math.random(), {
					headers: {
				...
				formData.append("crabvpn.js", new Blob([newCode], { type: "application/javascript+module" }), "crabvpn.js");
				...
				const errorMsg = err.message + " | در صورت عدم موفقیت، از طریق لینک زیر آپدیت کنید: https://github.com/Aryalleh/Pr";
		...
		if (url.pathname === "/api/restart-core" && request.method === "POST") {
			...
			try {
				const githubRes = await fetch("https://raw.githubusercontent.com/Aryalleh/Pr/refs/heads/main/crabvpn.js?t=" + Date.now(), {
					headers: {
						"Cache-Control": "no-cache, no-store, must-revalidate",
						Pragma: "no-cache",
						Expires: "0",
		...
		if (url.pathname === "/api/telegram/broadcast" && request.method === "POST") {
			const { message } = await request.json().catch(() => ({}));
			if (!message || !String(message).trim()) return new Response(JSON.stringify({ ok: false, error: "متن پیام اجباری است" }), { status: 400, headers: { "Content-Type": "application/json" } });
			const settingsRows = (await env.DB.prepare("SELECT key, value FROM settings WHERE key = 'telegram_bot_token'").all()).results || [];
			const botToken = settingsRows[0] && settingsRows[0].value;
			if (!botToken) return new Response(JSON.stringify({ ok: false, error: "توکن ربات تنظیم نشده" }), { status: 400, headers: { "Content-Type": "application/json" } });
			const { results } = await env.DB.prepare("SELECT telegram_user_id FROM users WHERE telegram_user_id IS NOT NULL AND telegram_user_id != ''").all();
			const chatIds = (results || []).map((r) => r.telegram_user_id);
			const broadcastTask = async () => {
				for (const chatId of chatIds) {
					try {
						await Telegram.call(botToken, "sendMessage", { chat_id: chatId, text: message });
					} catch (e) {}
					await new Promise((r) => setTimeout(r, 40));
				}
			};
			if (ctx) ctx.waitUntil(broadcastTask());
			else await broadcastTask();
			return new Response(JSON.stringify({ ok: true, queued: chatIds.length }), { headers: { "Content-Type": "application/json" } });
		}
...
const DbService = {
	async ensureSchema(db) {
      ...
      `,
		...
		try {
			await db.prepare("ALTER TABLE payments ADD COLUMN order_id TEXT").run();
		} catch (e) {}
		schemaEnsured = true;
	},
